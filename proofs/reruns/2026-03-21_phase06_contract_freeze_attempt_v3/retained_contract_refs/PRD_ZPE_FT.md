...
```mermaid
flowchart TD
    A["OHLCV bars / tick streams"] --> B["Tick-size normalization to integer units"]
    B --> C["Current transport path: delta + nibble + overflow codec"]
    B --> D["Current token path: directional quantization into 8-state token stream"]
    C --> E[".zpfin packet framing"]
    D --> F["Suffix array / KMP / Aho-Corasick pattern search"]
    C --> G["Current measured decode / roundtrip path"]
    D --> H["Pattern library and sidecar accelerator"]
    C --> I["Target unification layer: codec + tokenizer become one representation path"]
    D --> I
    I --> J["P16 or profile-variant extension"]
    I --> K["Hurst-adaptive threshold mode"]
    I --> L["Residual sidecar / compliance mode"]
    I --> M["TimescaleDB / ClickHouse / kdb adapters"]
    I --> N["Entropy / Wolfram / rolling-compression APIs"]
    I --> O["OPPM fuzzy search / TDA fingerprinting"]
    I --> P["Foundation-model pre-tokenizer bridge"]
```
